...
def get_ani_cam_name():
    ...
    for ass in lst:
        if "Shot" in ass.name() and ":" not in ass.name():
...
def main(check_maya_file):
    ...
    try:
        ...
        for _node in lst:
            all_node = cmds.listRelatives(_node, ad=1, ni=1)
            for _node_children in all_node:
                if "Geometry" in _node_children and "Shape" not in _node_children:
                    parent_node = cmds.listRelatives(_node_children, ap=1, f=1)[0]
                    parent_name = parent_node.split("|")[1]
                    root_str = "{}|{}".format(parent_node, _node_children)
