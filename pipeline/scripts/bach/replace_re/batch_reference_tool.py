...
# -*- encoding: utf-8 -*-
'''
File    :   BatchReplaceReference.py
Time    :   2022/11/01 18:35:01
Author  :   ZhenBao Liu
Version :   1.0
...
import batch_reference_com
# reload(batch_reference_com)

NAME = u"Batch replace reference - V001"
...
class BatchReplaceReference(QtWidgets.QMainWindow):
    def __init__(self, parent = None):
        super(BatchReplaceReference, self).__init__(parent)
        self.batch_com = batch_reference_com.ReplaceReferenceCom()
    ...
    def stuff_ui(self):
        ...
        self.batch_com.ani_file = self.lwt_ani_file
        self.batch_com.old_file = self.lwt_old_file
        self.batch_com.new_file = self.lwt_new_file
        self.btn_output_replace.clicked.connect(self.batch_com.run_reference)

    def btn_state(self, btn):
        if btn.isChecked() == True:
            self.lwt_old_file.setDisabled(True)
        else:
            self.lwt_old_file.setDisabled(False)

if __name__ == "__main__":
    app = QtWidgets.QApplication([])
    win = BatchReplaceReference()
    win.show()
    sys.exit(app.exec_())
