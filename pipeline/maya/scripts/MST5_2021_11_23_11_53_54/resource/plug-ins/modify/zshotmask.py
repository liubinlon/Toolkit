...
import maya.OpenMayaRender as omr_Old

import pymel.core as pmc
import maya.cmds as cmds

def getActiveCamera():
    activeCamera = om_Old.MDagPath()
    omui_Old.M3dView().active3dView().getCamera(activeCamera)
...
class ZShotMaskDrawOverride(omr.MPxDrawOverride):
    ...
    def prepareForDraw(self, obj_path, camera_path, frame_context, old_data):
        ...
        focalLength = cmds.getAttr(pmc.PyNode(getActiveCamera())+'.focalLength')
        data.text_fields[3] = u"焦距:{0}".format(str(round(focalLength,1)))
