...
# - add bone name ilter in "+ Bone" window
...
scriptPath = getCurrentGlobalScript['path'].rsplit('/',2)[0]+'/MST_DATA/scripts'
...
def addBoneUpdateJointList():
    ...
    filterWord = addBone_filterEdit.getText()
    ...
    if lodSourceMesh and lodBoneDict:
        ...
        for key in lodBoneDict.keys():
            ...
            #             mergeFullJointList.append(targetBone)
                        ...
                        # if not(bone in key or bone in processedChildBoneList):

                        #     # mergeSkinWeight( lodTargetMeshSkinCluster, bone, key )
                        #     #     pm.skinCluster( lodTargetMeshSkinCluster, edit = True, selectInfluenceVerts = bone )
                        #     # for vtx in pm.ls(sl=1, flatten = 1):
                        #     #     vtxs.append( vtx )
                        #     processedChildBoneList.append( bone )
                    #     pm.skinCluster( lodTargetMeshSkinCluster, edit = True, selectInfluenceVerts = key )
                    # for vtx in pm.ls(sl=1, flatten = 1):
                    #     vtxs.append( vtx )
                    # do weight move
                    ...
                    # pm.skinPercent( lodTargetMeshSkinCluster,
                    #                 vtxs,
                    #                 transformValue = [(targetBone,1.0)],
                    #                 normalize = True )
...
def mergeSkinWeight( skinCluster, scourceBone, targetBone ):
    #move all skin weight valve from a to b, keep the rest, normalize skin

    global mergeFullJointList

    # print scourceBone,targetBone

    pm.select( clear = True )

    if scourceBone in mergeFullJointList:
    ...
    # add targetBone into skin bone if not
    if not (targetBone in mergeFullJointList):
        skinCluster.addInfluence(targetBone)
        mergeFullJointList.append(targetBone)
    ...
    # lock bones
    pm.skinCluster( skinCluster, inf = scourceBone, edit = True, lockWeights = True )
    pm.skinCluster( skinCluster, inf = targetBone, edit = True, lockWeights = True )
...
def goShelf():
    ...
    pm.shelfButton( commandRepeatable = True, image1 = scriptPath+os.sep+"icons"+os.sep+"title2.png", label = "Skin Magic", parent = parentTab,
