...
SM_scriptPath = getCurrentGlobalScript['path'].rsplit('/',2)[0]+'/MST_DATA/scripts'
...
def springExtendChangeCmd( ignoreInputs = False ):
    SM_limitTextEditValue(springExtend_lineEdit, defaultValue = 0.0)

def springInertiaChangeCmd( ignoreInputs = False ):
    SM_limitTextEditValue(springInertia_lineEdit, defaultValue = 0.0)
...
def sm_createColPlaneCmd( ignoreInputs ):
    sm_createColPlane()
...
springInertia_lineEdit = pm.textField( SM_spring_tab + '|spring_groupBox|springInertia_lineEdit', edit = True )
...
springPoseMatch_checkBox = pm.checkBox( SM_spring_tab + '|spring_groupBox|keyRange_groupBox|springPoseMatch_checkBox', edit = True )
...
colPlane_subFix = "_SpringColPlane"
...
def offsetPosByDirection(sDirection,tPos,moveDistance):
    # get new target pos
    tPos += sDirection.normal()*moveDistance
...
def getVertexPositions(obj):
    vPositions = []
    for i in obj.vtx:
        vPositions.append(i.getPosition(space = 'world'))
    return vPositions

def checkPlaneCollision(objPos, refLocPos, colPlane):

    v_coords = getVertexPositions(colPlane)

    # get obj distance to plane
    n = dt.Vector(pm.xform(colPlane,worldSpace =1,matrix =1,q=1)[4:7])    # Y axis direction of plane
    q = v_coords[1]
    d = n.dot(q)
    pt = objPos
...
def sm_repeatMoveToPlane(obj, objPos, objTarget, colPlane, times):
    n = dt.Vector(pm.xform(colPlane,worldSpace =1,matrix =1,q=1)[4:7])    # Y axis direction of plane
    q = get_trans(colPlane)
    d = n.dot(q)
    # for i in range(times):
    #     pt = objPos
    #     obj.setTranslation(proj_pt_to_plane(pt, n, d), space='world')
    #     if (i+1) != times:
    #         obj.setTranslation(get_trans(objTarget), space='world')
    pt = objPos
    outPos = proj_pt_to_plane(pt, n, d)
    return outPos
...
def sm_createColPlane():

    # remove exist plane
    if get_node('*'+colPlane_subFix+'*'):
        pm.delete(get_node('*'+colPlane_subFix+'*'))

    colPlane = pm.polyPlane(name="the"+colPlane_subFix, sx=1,sy=1,w=10,h=10,ch=1)[0]
    pm.setAttr(colPlane.doubleSided, False)     # one side display

    # lock scale
    pm.setAttr(colPlane.sx, lock=True)
    pm.setAttr(colPlane.sy, lock=True)
    pm.setAttr(colPlane.sz, lock=True)
...
def springPrepare():
    ...
    isPoseMatch = bool(springPoseMatch_checkBox.getValue())    
    ...
    colPlaneLst = [get_node('*'+colPlane_subFix+'*')]
    ...
    mass = float(springInertia_lineEdit.getText())
...
def SpringMagicMaya(
    ...
    while iii < ((endFrame-startFrame)+(1/subDiv)):

        # print 'Frame',i

        pm.currentTime( startFrame+iii, edit=True )
        # calculate obj
        for obj in objs:
            ...
            if len(obj.listRelatives(children = True))>0 and (obj.getParent() is not None):     # skip end bone and root bone
                ...
                if child:
                    ...
                    if iii > 0 or isSeconendPath:     # only skip first frame of first calculation path
                        ...
                        if mass > 0.0:
                            ...
                            smBoneRefLocOffsetDistance = ((smBoneRefLocOffsetDir)*(1-ratio)*(1-mass)).length()
                        ...
                        isPlaneHit = False
                        if isCollision and colPlaneLst:
                            colPlane = colPlaneLst[0]
                            colResult_Plane = checkPlaneCollision(cur_obj_pos, cur_child_pos, colPlane)
                            isCollidePlane_Parent = False
                            if obj.getParent() in obj_trans_dict.keys():
                                isCollidePlane_Parent = obj_trans_dict[obj.getParent()][9]
                            if colResult_Plane or isCollidePlane_Parent:
                                cur_child_pos = sm_repeatMoveToPlane(obj, cur_child_pos, child, colPlane, 3)
                                isPlaneHit = True
                        ...
                        bonePreDis = (prev_child_pos - prevprev_child_pos).length()
                        obj_trans_dict[obj] = [cur_obj_pos,cur_child_pos,r,cur_grandChild_pos,hasChildCollide,boneLength,upVector,prev_child_pos,bonePreDis,isPlaneHit]
                        ...
                        if isPoseMatch:
                            ...
                            while ttt < (endFrame-startFrame):
                                pm.currentTime( startFrame+ttt, edit=True )
                                null.setTranslation( child.getTranslation(space='world'), space='world')
                                pm.setKeyframe(null, attribute = 'translate')
                                null.setRotation( child.getRotation(space='world'), space='world')
                                pm.setKeyframe(null, attribute = 'rotate')
                                ttt += 1
                        else:
                        ...
                        obj_trans_dict[obj] = [prev_obj_pos, prev_child_pos, prev_rot, prev_grandChild_pos, False, boneLength,prev_obj_yAxis,prev_child_pos,0.0,False]
        ...
        if isSeconendPath:
            kk = 50.0
        SM_setProgressBar(SM_main_progressBar,float(iii)/jj/float(endFrame-startFrame+1)*100+kk)

        # print isLoop, startFrame+i+1, endFrame
        if isLoop and (startFrame+iii > endFrame) and isSeconendPath == False: # at the end frame of first path
            iii = 0   # back to first frame
