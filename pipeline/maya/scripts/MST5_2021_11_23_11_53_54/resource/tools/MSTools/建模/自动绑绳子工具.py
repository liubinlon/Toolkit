#coding:gbk
# Compiled at: 2018-09-07 09:04:29
...
def CreateDynamicRope():
    global DynamicCurv
    global DynamicCurvControl_N
    global DynamicCurvControl_V
    global DynamicCurvPivotN
    global DynamicCurvPivotV
    global DynamicNucleusNode
    global Dynamic_Group
    global HairSystemNode
    global InitialCurv
    global NewCurvesDynamicNode
    global nRigidNode
    global the_cir
    global the_nub
    global the_nurbsTesse
    global the_poly
    ...
    mc.rebuildCurve(NewCurv, ch=True, rpo=True, rt=False, end=True, kr=False, kcp=False, kep=True, kt=False, s=200, d=3, tol=0.01)
    ...
    DynamicCurvControl_N = mc.polySphere(r=CurveRadius * 1.5, sx=20, sy=20, ax=(0,
                                                                                1,
                                                                                0), cuv=2, ch=1)[0]
    ...
    DynamicCurvControl_V = mc.polySphere(r=CurveRadius * 1.5, sx=20, sy=20, ax=(0,
                                                                                1,
                                                                                0), cuv=2, ch=1)[0]
    ...
    the_cir = mc.circle(c=(0, 0, 0), nr=(0, 1, 0), sw=360, r=CurveRadius, d=3, ut=0, tol=0.01, s=8, ch=1, n=DynamicCurv + '_circle')[0]
    mc.select(DynamicCurv, add=True)
    the_nub = mc.extrude(the_cir, DynamicCurv, ch=True, rn=False, po=0, et=2, ucp=1, fpt=1, upn=1, rotation=0, scale=1, rsp=1, n=DynamicCurv + '_nub')[0]
    mc.setAttr(the_cir + '.visibility', 0)
    mc.setAttr(the_nub + '.visibility', 0)
    the_poly = mc.nurbsToPoly(the_nub, mnd=1, n=DynamicCurv + '_poly', ch=1, f=2, pt=1, pc=200, chr=0.9, ft=0.01, mel=0.001, d=0.1, ut=1, un=CurveUnumber, vt=1, vn=CurveVnumber, uch=0, ucr=0, cht=0.2, es=0, ntr=0, mrt=0, uss=1)[0]
...
def DynamicNucleusSubsteps():
    if mc.objExists('*Dynamic_G'):
        if mc.objExists(DynamicNucleusNode):
            NewDynamicNucleusSubsteps = mc.intSliderGrp('New_DynamicNucleusSubsteps_Value', q=True, v=True)
            mc.setAttr(DynamicNucleusNode + '.subSteps', NewDynamicNucleusSubsteps)
...
def MayaDynamicRope_ChineseHelp():
    ...
    MayaDynamicRope_ChineseHelp_Tool_UI_Wind = mc.window('MayaDynamicRope_ChineseHelp_Tool', title='MayaDynamicRope_ChineseHelp_Tool')
    mc.columnLayout(adjustableColumn=True, bgc=(0.13, 0.13, 0.13), w=500, h=310, cal='left', cat=('left',
                                                                                                  10))
    HelpH = 20
    mc.text(label='', h=15)
    mc.text(label=u'Radius:     ', h=HelpH)
    mc.text(label=u'U_Number:   ', h=HelpH)
    mc.text(label=u'V_Number:   ', h=HelpH)
    mc.text(label='', h=15)
    mc.text(label=u'SubSteps:   ', h=HelpH)
    mc.text(label=u'Collision:  ', h=HelpH)
    mc.text(label='', h=15)
    mc.text(label=u'Friction:   ', h=HelpH)
    mc.text(label=u'Stretch:   ', h=HelpH)
    mc.text(label='', h=15)
    mc.text(label=u'Create Dynamic Rope:   ', h=HelpH)
    mc.text(label=u'Initial Dynamic:       ', h=HelpH)
    mc.text(label=u'Delete  Dynamic:       ', h=HelpH)
    mc.text(label=u'Finish:      ', h=HelpH)
    mc.text(label=u'QQ 2384230095', h=HelpH)
    mc.text(label='', h=15)
...
def MayaDynamicRope_Tool():
    ...
    MayaDynamicRope_Tool_UI_Wind = mc.window('MayaDynamicRope_Tool', title='MayaDynamicRope_Tool 1.0', menuBar=True, bgc=(0.1,
                                                                                                                          0.1,
                                                                                                                          0.1))
    ...
    mc.floatSliderGrp('New_CurveRadius_Valu', label=u'Radius', cw3=(70, 40, 105), field=True, min=0, max=100, v=CurveRadius_Value, cc='CurveRadius()')
    mc.text(label='', h=5)
    mc.setParent('..')
    mc.rowLayout(nc=3, adj=1)
    ...
    mc.intSliderGrp('New_CurveUnumber_Value', label=u'U_Number', cw3=(70, 40,
                                                                            105), field=True, min=0, max=100, v=CurveUnumber_Value, cc='CurveUnumber()')
    mc.text(label='', h=5)
    mc.setParent('..')
    mc.rowLayout(nc=3, adj=1)
    ...
    mc.intSliderGrp('New_CurveVnumber_Value', label=u'V_Number', cw3=(70, 40,
                                                                            105), field=True, min=0, max=1000, v=CurveVnumber_Value, cc='CurveVnumber()')
    mc.text(label='', h=10)
    mc.setParent('..')
    mc.text(label='', h=10)
    mc.rowLayout(nc=3, adj=1)
    ...
    mc.intSliderGrp('New_DynamicNucleusSubsteps_Value', label=u'SubSteps', cw3=(70,
                                                                                      40,
                                                                                      105), field=True, min=0, max=20, v=DynamicNucleusSubsteps_Value, cc='DynamicNucleusSubsteps()')
    mc.text(label='', h=5)
    mc.setParent('..')
    mc.rowLayout(nc=3, adj=1)
    ...
    mc.intSliderGrp('New_DynamicNucleusMaxCollision_Value', label=u'Collision', cw3=(70,
                                                                                           40,
                                                                                           105), field=True, min=0, max=20, v=DynamicNucleusMaxCollision_Value, cc='DynamicNucleusMaxCollision()')
    mc.text(label='', h=5)
    mc.setParent('..')
    mc.text(label='', h=10)
    mc.rowLayout(nc=3, adj=1)
    ...
    mc.floatSliderGrp('New_HairSystemSelfFriction_Value', label=u'Friction', cw3=(70,
                                                                                        40,
                                                                                        105), field=True, min=0, max=1, v=SelfFriction_Value, cc='HairSystemSelfFriction()')
    mc.text(label='', h=5)
    mc.setParent('..')
    mc.rowLayout(nc=3, adj=1)
    Stretch_Value = 10
    ...
    mc.floatSliderGrp('New_HairSystemStretch_Value', label=u'Stretch', cw3=(70,
                                                                                  40,
                                                                                  105), field=True, min=0, max=500, v=Stretch_Value, cc='HairSystemStretch()')
    mc.text(label='', h=5)
    mc.setParent('..')
    mc.text(label='', h=5)
    ...
    mc.button(label=u'Create Dynamic Rope', command='CreateDynamicRope()', bgc=(0,
                                                                                0,
                                                                                0))
    mc.setParent('..')
    mc.text(label='', h=5)
    mc.rowLayout(nc=3)
    mc.button(label=u'Initial Dynamic', w=150, command='RestoreInitial()', bgc=(0,
                                                                                0,
                                                                                0))
    mc.button(label=u'Delete  Dynamic', w=150, command='DeleteDynamic()', bgc=(0, 0,
                                                                               0))
    mc.setParent('..')
    mc.text(label='', h=5)
