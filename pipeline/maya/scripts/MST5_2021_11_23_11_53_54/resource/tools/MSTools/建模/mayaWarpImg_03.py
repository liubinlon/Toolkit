...
#0.0.4:防止增加无用的历史记录，与输出后是否启用历史的选项

#--------------------------------
#本应用基本上等于是面向过程,使用类来封装为了避免覆盖其他应用的函数
#由于maya直接使用unicode的文字会无法正确显示,应该使用[u'string']的形式
#--------------------------------
# 将py文件放在maya/script目录下
...
#使用官方的mel方式开启与关闭历史
def mwi_history (*args):
	if args[0] == True or args[0] >= 1:
		mm.eval("constructionHistory -toggle true;updateConstructionHistory;autoUpdateAttrEd;")
	elif args[0] == False or args[0] == 0:
		mm.eval("constructionHistory -toggle false;updateConstructionHistory;autoUpdateAttrEd;")
...
class mayaWarpUI :
	...
	def __workButtonPressed(self,*args):
		...
		if self.chkBoxTile.getValue():
			...
			chkTile = False
		if self.chkBoxHistory.getValue():
			print u'当前对输出后开启历史记录!'
			chkHistory = True
		else:
			print u'当前对输出后关闭历史记录!'
			chkHistory = False
		...
		Over=chkOver, Tile=chkTile, rx=self.rx, ry=self.ry, history=chkHistory).warpImg()
	...
	def showWindow(self):
		...
		self.chkBoxHistory = pm.checkBox(label = u'输出后开启历史', value=True, parent=layout)
		#按钮
...
class mayaWarpImg :
	...
	noAlpha=False, Over=True, Tile=True, rx=2048, ry=2048, history=True):
		__author__ = 'MineClever'
		__contact__ = 'chnisesbandit@live.cn'
		__website__ = 'https://steamcommunity.com/id/chinsesbandit/home'
		#定位当前工程文件夹的images目录
	...
	def __doWarp ( self, obj, file, ftype) :
		...
		iuv='mwi_showUV', ouv='mwi_refUV',bg=(0,0,0))
		d=u'已输出%s\n'%fExportName
		print (d)
		pm.warning(d)
	...
	def __get_objColor(self):
		pass

	def warpImg(self):
		#强制关闭历史记录
		mwi_history(False)
		print(u'处理时，已经强制关闭历史记录')
		#传入选择的对象
		try:
...
class mayaWarp_UV_Prefix :
	...
	def copyUV (self,uv2):
		...
		print u'完成预设操作'
		TempHistory = pm.constructionHistory( q=True, tgl=True )
		print (u'历史记录状态为'+str(TempHistory))
		mwi_history(False)
		...
		mwi_history(TempHistory)
		print(u'历史记录状态已恢复为'+str(TempHistory))
