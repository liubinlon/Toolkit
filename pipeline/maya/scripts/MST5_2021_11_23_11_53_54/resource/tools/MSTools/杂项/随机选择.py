...
def sel_Bfb():
    ...
    if selnum >= len(selold):
        cmds.warning(u'随机个数大于选择的个数，请重新设置随机个数')
        return ''
...
window = cmds.window("random_select_xj", title="随机选择", iconName='Short Name', widthHeight=(300, 220) ,menuBar=True)
...
cmds.rowLayout(nc=2)
cmds.button('buton1', label='开启',w=150,bgc=(0,0,0), c='sel_Bfb_Job()\nButton_Color("buton1","butoff1","On")')
cmds.button('butoff1', label='关闭',w=150,bgc=(0,0,0), c='Del_Bfb_Job()\nButton_Color("buton1","butoff1","Off")')
cmds.setParent( '..' )
cmds.text(l="\n填入要随机选取物体的个数")
...
cmds.rowLayout(nc=2)
cmds.button('buton2', label='开启',w=150,bgc=(0,0,0), c='sel_Num_Job()\nButton_Color("buton2","butoff2","On")')
cmds.button('butoff2', label='关闭',w=150,bgc=(0,0,0), c='Del_Num_Job()\nButton_Color("buton2","butoff2","Off")')
cmds.setParent( '..' )
cmds.setParent( '..' )
#-------------------------------------
