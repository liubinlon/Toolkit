...
class sp3dObjectList (object):
    ...
    authType = {
        'default': ('mesh',),
        'source': ('mesh', 'locator','aiStandIn','assemblyReference', 'witGpuCache', 'transform'),
        'target': ('mesh')
    };
...
class spPaint3dWin (object):
    ...
    def __init__(self):
        ...
        self.radiusOptions = [False, 30, 1, 10]
        #----------------------
        # Target Surface(s)
        #----------------------
        ...
        # arthur code
        self.uiPaintMetricsSep3 = mc.separator(w=250)
        self.Splatter = mc.checkBox(label='Splatter', ann='Activate Splatter', changeCommand=lambda * args:self.uiRadiusCallback())
        self.radius = mc.intSliderGrp(field=True, label='Radius', minValue=0, maxValue=300, fieldMinValue=0,
                                      fieldMaxValue=100000, value=30, changeCommand=lambda * args:self.uiRadiusCallback())

        self.numRange = mc.intFieldGrp(numberOfFields=2, label='Min', extraLabel='Max', value1=1, value2=10,
                                       changeCommand=lambda *args: self.uiRadiusCallback())

        mc.formLayout(self.uiPaintMetricsForm, edit=True,
                      attachControl=[(self.uiPaintMetricsSep3, 'top', 5, self.uiJitterFieldV),
                                     (self.Splatter, 'top', 5, self.uiPaintMetricsSep3),
                                     (self.radius, 'top', 5, self.uiPaintMetricsSep3),
                                     (self.radius, 'left', 0, self.Splatter),
                                     (self.numRange, 'top', 5, self.Splatter)
                                     ])
    ...
    def uiRadiusCallback(self, *args):
        self.radiusOptions = [mc.checkBox(self.Splatter, q=True, v=True),
                              mc.intSliderGrp(self.radius, q=True, v=True),
                              mc.intFieldGrp(self.numRange, q=True, v1=True),
                              mc.intFieldGrp(self.numRange, q=True, v2=True)]
    ...
    def updateCtx(self):
        ...
        if (self.ctx):
            ...
            self.ctx.runtimeUpdate(self.uiValues, self.transform, self.sourceList, self.targetList, self.radiusOptions);
...
def getDAGPath(node, depth=False):
    ...
    if(nodetype == 'transform' or nodetype == "assemblyReference"):
        ...
        if(childlist):
            if(len(childlist) == 1):
                ...
                if(depth):
                    dag = mc.listRelatives(node, fullPath=True, shapes=True)
                else:
                    dag = mc.listRelatives(node, path=True, shapes=True)
        else:
            dag = mc.listRelatives(node, children=True)
    if(dag):
        return dag[0]
    else:
        return dag
