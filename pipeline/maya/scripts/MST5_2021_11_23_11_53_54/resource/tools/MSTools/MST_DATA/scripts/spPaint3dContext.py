...
class paintContext (object):
    ...
    define paintContext
    '''
    def __init__(self, uioptions, transformoptions, sourcelist, targetlist, radius):
        '''
        initial setup
        '''
        ...
        self.runtimeUpdate(uioptions, transformoptions, sourcelist, targetlist, radius)
    ...
    def onPress(self):
        ...
        # print '----------spPaint3dContextID' ,type(spPaint3dContextID)
        ...
        if self.radius[0]:
            ...
            for i in range(random.randint(self.radius[2], self.radius[3])):
                x_va = random.randint(-radius, radius)
                y_va = random.randint(-radius, radius)
                new_pos_x = x_va + pressPosition[0]
                new_pos_y = y_va + pressPosition[1]

                new_pos_x = new_pos_x if new_pos_x > 0 else 1
                new_pos_y = new_pos_y if new_pos_y > 0 else 1

                # print new_pos_x, new_pos_y, pressPosition

                worldPos, worldDir = getViewportClick(new_pos_x, new_pos_y)

                # print worldPos, worldDir
            ...
            if (intersected):
                ...
                intersected.isValid(True)
                if (sp3d_dbg): logDebugInfo('found intersected')
                if (sp3d_log): print ('intersection at X: %f | Y: %f | Z: %f' % (
                ...
                if (self.uiValues.random):
                    intersected.dagMeshSourceObject = self.sourceList.getRandom()
                else:
    ...
    def onDrag(self):
        ...
        if self.radius[0]:
            ...
            for i in range(random.randint(self.radius[2], self.radius[3])):
                x_va = random.randint(-radius, radius)
                y_va = random.randint(-radius, radius)
                new_pos_x = x_va + dragPosition[0]
                new_pos_y = y_va + dragPosition[1]
            ...
            if (intersected):
                ...
                if (len(self.strokeIntersectionList.intersectionList) == 0):
                    ...
                    if (self.uiValues.paintFlux):
                        ...
                        distanceToPrevious = getDistanceBetween(
                            self.strokeIntersectionList.intersectionList[-1].hitPoint, intersected.hitPoint)
                        correctedPaintDistance = self.uiValues.paintDistance
                        # correctedPaintDistance = getCorrectedDistance(self.uiValues.paintDistance, self.unit)
                        if (sp3d_log): print (
                        'intersection at X: %f | Y: %f | Z: %f |||| distance from previous: %f (x: %f | y: %f | z: %f)(threshold: %f)(length of list: %i)' % (
                        intersected.hitPoint.x, intersected.hitPoint.y, intersected.hitPoint.z,
                        getDistanceBetween(self.strokeIntersectionList.intersectionList[-1].hitPoint,
                        ...
                        self.strokeIntersectionList.intersectionList[-1].hitPoint.z, correctedPaintDistance,
                        len(self.strokeIntersectionList.intersectionList)))
                        if (distanceToPrevious < correctedPaintDistance):
                            # distance with the new intersection is below threshold, forcing an end of proc
                            if sp3d_log: print ("breaking from onDrag since < threshold")
                        ...
                        # paintFlux set on timer
                        if (mc.timerX(startTime=self.strokeIntersectionList.intersectionList[
                            -1].timestamp) < self.uiValues.paintTimer):
                            # timer with the new intersection is below threshold, forcing an end of proc
                            intersected.isValid(False)
                        else:
                            # this intersection is valid, starting its timer
                ...
                if intersected.isValid():
                    ...
                    # feed various data into the intersectionPoint object
                    if (self.uiValues.random):
                        intersected.updateDAGSourceObject(self.sourceList.getRandom())
                    else:
    ...
    def runtimeUpdate(self, uioptions, transformoptions, sourcelist, targetlist, radius):
        '''
