...
class CpCurveBevel(MayaNodeT):
    ...
    def compute(self, plug, data):
        ...
        if curveDegree == 1:
            knots = [float(i) for i in xrange(numcv)]
            ...
            if curveForm == curveFn.kPeriodic:
                knots[:curveDegree] = [
                    float(i) for i in reversed(xrange(0, -curveDegree, -1))
                ]
                knots[curveDegree:] = [
                    float(i) for i in xrange(1,
...
class CpCurveToPoly(MayaNodeT):
    ...
    def compute(self, plug, data):
        ...
        meshFn = om.MFnMesh()
        meshFn.create(
            polyPts, [len(polyPts)], [i for i in xrange(len(polyPts))],
