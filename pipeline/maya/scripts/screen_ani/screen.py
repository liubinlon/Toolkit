...
class btn:
    ...
    def add_Display(self):
        cmds.headsUpDisplay('cameraName', section=2, block=1, blockSize='large', label='camera:', labelFontSize='large', dataFontSize="large",command=self.get_camera, event='SelectionChanged', nodeChanges='attributeChange')
        cmds.headsUpDisplay('focalLength', section=5, block=1, blockSize='large', label='focal length:', labelFontSize='large', dataFontSize="large", command=self.get_focal_length, attachToRefresh=True)
        cmds.headsUpDisplay('displaySize', section=4, block=1, blockSize='large', label='displaySize:', labelFontSize='large', dataFontSize="large", command=self.get_proj_size, attachToRefresh=True)
        cmds.headsUpDisplay('currentTime', section=7, block=1, blockSize='large', label='  ', labelFontSize='large', dataFontSize="large", command=self.get_time, attachToRefresh=True)
        cmds.headsUpDisplay('currentfps', section=9, block=1, blockSize='large', label='fps:', labelFontSize='large', dataFontSize="large", command=self.get_current_fps, attachToRefresh=True)
        cmds.headsUpDisplay('artist', section=0, block=1, blockSize='large', label='artist:', labelFontSize='large', dataFontSize="large", command=self.get_user_name, attachToRefresh=True)
    ...
    def setwidthValue(self):
        ...
        return [int(widthValue), int(heightValue)]
    ...
    def playblast(self):
        ...
        self.remdisplay()
        self.add_Display()
        dir_path = str(pm.sceneName().dirname())
        newpath = self.openFileDialog(dir_path)
        for_mat = self.setformat()
        # filename = os.path.basename(__file__)
        file_name = self.autofilename()
        # information = self.set_information()
        _filename = newpath + "/" + file_name + "." + for_mat[1]
        get_size_value = float(self.getfonSizeValue())
        percent_data = int(get_size_value*100)
        cmds.playblast(format=for_mat[0], filename=_filename, sequenceTime=0, forceOverwrite=True, clearCache=1, viewer=1, showOrnaments=1, compression="PNG", percent=percent_data, quality=100, widthHeight=self.setwidthValue())
